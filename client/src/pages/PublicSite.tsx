...
function ContactSection() { return <section className="public-contact"><div><div className="section-kicker">08 · Start here</div><h2>Have something worth<br /><i>putting into the world?</i></h2><p>Tell us what you are building, changing, launching, or becoming. We will help you turn it into a presence people can feel.</p></div><form onSubmit={(event) => { event.preventDefault(); alert("Thanks — the AgencyOS team will follow up shortly."); }}><label>Name<input required placeholder="Your name" /></label><label>Work email<input required type="email" placeholder="you@agency.com" /></label><label>What are you looking for?<textarea required placeholder="A website, personal brand, social media, PR, content..." /></label><button className="public-button public-button--dark" type="submit">Send the request <ArrowRight size={15} /></button></form></section>; }
function InteriorPage({ page, title }: { page: string; title: string }) { return <main className="public-interior"><div className="section-kicker">AgencyOS / {page.replaceAll("-", " ")}</div><h1>{title}</h1><p className="interior-lede">A media agency for brands, founders, artists, and teams who want to look sharper, sound clearer, and be seen by the right people.</p><div className="interior-grid">{(page === "services" ? serviceGroups.flatMap((group) => group.links) : page === "blog" ? articles.map((article) => article[1]) : ["Websites and landing pages", "Personal branding", "Social media management", "Content strategy and production", "Public relations", "Influencer campaigns"]).map((item, index) => <div className="interior-card" key={item}><span>{String(index + 1).padStart(2, "0")}</span><h2>{item}</h2><p>See how we turn this part of your public presence into something clear, considered, and memorable.</p><Link href="/contact" className="text-link">Talk to us <ArrowRight size={14} /></Link></div>)}</div></main>; }
